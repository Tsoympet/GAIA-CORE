...
class CloudFallbackClient:
    ...
    async def generate(self, prompt: str, model_id: str, **kwargs: object) -> str:
        if not self.config.enabled:
            raise PermissionError("cloud model fallback is disabled by configuration")
        raise NotImplementedError(
            f"cloud provider {self.config.provider!r} is configured but no adapter has been installed"
        )
